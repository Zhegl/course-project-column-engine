...
std::string ColumnTypeString::GetTypeName() const {
    return "string";
}
...
size_t ColumnTypeString::WriteType(const std::vector<ColumnValue>& data, FileWriter& writer) {
    ...
    for (const auto& val : data) {
        ...
        if (dict.find(s) == dict.end()) {
            dict[s] = static_cast<int64_t>(dict.size());
            reverse_dict.emplace_back(s);
            words_size += static_cast<uint32_t>(s.size());
            bloom.Add(s);
        }
    }
...
std::string ColumnTypeInt64::GetTypeName() const {
    return "int64";
}
...
ColumnValue ColumnTypeInt64::ConvertType(std::string val) {
    try {
        ...
        throw std::runtime_error("\"" + val + "\" is not a valid int64");
    }
}
...
size_t ColumnTypeInt64::WriteType(const std::vector<ColumnValue>& data, FileWriter& writer) {
    ...
    for (size_t i = 0; i < data.size(); ++i) {
        const int64_t v = std::get<int64_t>(data[i]);
        if (i % block_size == 0 || min_val[i / block_size] > v) {
            min_val[i / block_size] = v;
        }
        if (i % block_size == 0 || max_val[i / block_size] < v) {
            max_val[i / block_size] = v;
        }
        unique.insert(v);
    }
