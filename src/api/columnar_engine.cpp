...
ApiPipeline::ApiPipeline(std::unique_ptr<internal::Pipeline> impl)
    : impl_(std::move(impl)) {}
...
ApiPipeline ApiPipeline::Count(std::string arg) {
    return ApiPipeline(std::make_unique<internal::Pipeline>(impl_->Count(std::move(arg))));
}

ApiPipeline ApiPipeline::Where(std::string arg) {
    return ApiPipeline(std::make_unique<internal::Pipeline>(impl_->Where(std::move(arg))));
}

ApiPipeline ApiPipeline::Aggregate(std::string arg) {
    return ApiPipeline(std::make_unique<internal::Pipeline>(impl_->Aggregate(std::move(arg))));
}

ApiPipeline ApiPipeline::Limit(size_t arg) {
    return ApiPipeline(std::make_unique<internal::Pipeline>(impl_->Limit(arg)));
}

ApiPipeline ApiPipeline::Offset(size_t arg) {
    return ApiPipeline(std::make_unique<internal::Pipeline>(impl_->Offset(arg)));
}

ApiPipeline ApiPipeline::OrderBy(std::string arg) {
    return ApiPipeline(std::make_unique<internal::Pipeline>(impl_->OrderBy(std::move(arg))));
}

ApiPipeline ApiPipeline::Rename(std::string from, std::string to) {
    return ApiPipeline(std::make_unique<internal::Pipeline>(impl_->Rename(std::move(from), std::move(to))));
}

ApiPipeline ApiPipeline::Add(std::string arg) {
    return ApiPipeline(std::make_unique<internal::Pipeline>(impl_->Add(std::move(arg))));
}

ApiPipeline ApiPipeline::Case(std::string name, std::string when_cond, std::string then_expr, std::string else_expr) {
    return ApiPipeline(std::make_unique<internal::Pipeline>(
...
ApiPipeline ApiPipeline::GroupByAggregateImpl(std::vector<std::string> group_columns, std::string aggregates) {
    ...
    return ApiPipeline(std::make_unique<internal::Pipeline>(impl_->GroupByAggregateVec(std::move(all_args))));
}

ApiPipeline ApiPipeline::SelectImpl(std::vector<std::string> columns) {
    return ApiPipeline(std::make_unique<internal::Pipeline>(impl_->SelectVec(columns)));
}
...
ApiPipeline ColumnEngine::Api() {
    return ApiPipeline(std::make_unique<internal::Pipeline>(impl_->Api()));
}
