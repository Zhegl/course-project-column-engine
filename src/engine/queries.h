...
public:
    ...
    ColumnValue Get(EngineBatch&, RowIndex) override { return val_; }
    std::string GetName() override { return "'" + val_ + "'"; }
private:
    std::string val_;
};
...
public:
    ...
    std::string GetName() override { return col_name_; }
private:
    size_t col_idx_;
...
public:
    RegexpReplace(size_t col_idx, std::string col_name, std::string pattern, std::string replacement)
        : col_idx_(col_idx), col_name_(std::move(col_name)),
          pattern_(pattern), re_(pattern), replacement_(std::move(replacement)) {}
    ColumnValue Get(EngineBatch& batch, RowIndex i) override {
        const std::string& s = std::get<std::vector<std::string>>(batch.columns[col_idx_])[i];
        return std::regex_replace(s, re_, replacement_);
    }
    std::string GetName() override { return "regexp_replace(" + col_name_ + ", '" + pattern_ + "', '" + replacement_ + "')"; }
private:
    size_t col_idx_;
    std::string col_name_;
    std::string pattern_;
