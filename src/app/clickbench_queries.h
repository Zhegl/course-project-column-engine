...
static QueryResult RunQuery(Engine& engine, int query_num) {
    switch (query_num) {
        ...
        case 26:
            return engine.Api()
                .Where("SearchPhrase <> ''")
                .OrderBy("EventTime ASC, SearchPhrase ASC")
                .Limit(10)
                .Select("SearchPhrase")
                .Run();
        case 25:
        ...
        case 28:
            return engine.Api()
                .Where("Referer <> ''")
                .Add("regexp_replace(Referer, '^https?://(?:www\\.)?([^/]+)/.*$', '$1')")
                .Add("length(Referer)")
                .GroupByAggregate("regexp_replace(Referer, '^https?://(?:www\\.)?([^/]+)/.*$', '$1')", "AVG(length(Referer)), COUNT(*), MIN(Referer)")
                .Rename("AVG(length(Referer))", "l")
                .Rename("COUNT(*)", "c")
                .Where("c > 100000")
                .OrderBy("l DESC")
                .Limit(25)
                .Select("regexp_replace(Referer, '^https?://(?:www\\.)?([^/]+)/.*$', '$1')", "l", "c", "MIN(Referer)")
                .Run();
        case 27:
            return engine.Api()
                .Where("URL <> ''")
                .Add("length(URL)")
                .GroupByAggregate("CounterID", "AVG(length(URL)), COUNT(*)")
                .Rename("AVG(length(URL))", "l")
                .Rename("COUNT(*)", "c")
                .Where("c > 100000")
                .OrderBy("l DESC")
                .Limit(25)
                .Select("CounterID", "l", "c")
                .Run();
        case 29:
            return engine.Api()
        ...
        case 39:
            return engine.Api()
                ...
                .Where("EventDate <= '2013-07-31'")
                .Where("IsRefresh = 0")
                .Case("tmp", "SearchEngineID = 0", "Referer", "''")
                .Case("Src", "AdvEngineID = 0", "tmp", "''")
                .GroupByAggregate("TraficSourceID", "SearchEngineID", "AdvEngineID", "Src", "URL", "COUNT(*)")
                .Rename("COUNT(*)", "PageViews")
                .Rename("URL", "Dst")
                .OrderBy("PageViews DESC")
                .Offset(1000)
                .Limit(10)
                .Select("TraficSourceID", "SearchEngineID", "AdvEngineID", "Src", "Dst", "PageViews")
                .Run();
        case 40:
            return engine.Api()
                .Where("CounterID = 62")
        ...
        case 41:
            return engine.Api()
                ...
                .GroupByAggregate("WindowClientWidth", "WindowClientHeight", "COUNT(*)")
                .Rename("COUNT(*)", "PageViews")
                .OrderBy("PageViews DESC")
                .Offset(10000)
                .Limit(10)
                .Select("WindowClientWidth", "WindowClientHeight", "PageViews")
                .Run();
        case 42:
            return engine.Api()
                .Where("CounterID = 62")
